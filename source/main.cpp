...
static constexpr size_t GEOM_CULL_BATCH_DISPATCH_CMDS_UAV_DESCRIPTOR_SLOT = 2;
...
static std::array<vkn::Buffer, GEOM_QUEUE_COUNT> s_geomBatchDispatchCmdBuffer;
static std::array<vkn::Buffer, GEOM_QUEUE_COUNT> s_geomDrawCmdGenDispatchCmdBuffer;
...
static void CreateGeomCullingAndInstancingResources()
{
    ...
    for (size_t queue = 0; queue < GEOM_QUEUE_COUNT; ++queue) {
        ...
        s_geomBatchDispatchCmdBuffer[queue].Create(
            &s_vkDevice,
            sizeof(COMMON_CMD_DISPATCH_INDIRECT),
            VK_BUFFER_USAGE_2_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_2_INDIRECT_BUFFER_BIT,
            allocInfo
        );
        s_vkDevice.SetObjDebugName(s_geomBatchDispatchCmdBuffer[queue], "%s_GEOM_BATCH_DISPATCH_CMD_BUFFER", GEOM_QUEUE_DBG_NAMES[queue]);
        
        s_geomDrawCmdGenDispatchCmdBuffer[queue].Create(
            &s_vkDevice,
            sizeof(COMMON_CMD_DISPATCH_INDIRECT),
            VK_BUFFER_USAGE_2_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_2_INDIRECT_BUFFER_BIT,
            allocInfo
        );
        s_vkDevice.SetObjDebugName(s_geomDrawCmdGenDispatchCmdBuffer[queue], "%s_GEOM_DRAW_CMD_GEN_DISPATCH_CMD_BUFFER", GEOM_QUEUE_DBG_NAMES[queue]);
...
static void WriteGeomCullingDescriptorSet(GEOM_QUEUE queue)
{
    ...
    s_descriptorBuffer.WriteDescriptor(DESC_SET_ID_GEOM_CULLING, GEOM_CULL_VIS_INST_ID_QUEUES_UAV_DESCRIPTOR_SLOT, 
        queue, s_visGeomIDQueueBuffer[queue]);

    s_descriptorBuffer.WriteDescriptor(DESC_SET_ID_GEOM_CULLING, GEOM_CULL_VIS_INST_ID_QUEUE_SIZES_UAV_DESCRIPTOR_SLOT, 
        queue, s_visGeomIDQueueSizeBuffer[queue]);

    s_descriptorBuffer.WriteDescriptor(DESC_SET_ID_GEOM_CULLING, GEOM_CULL_BATCH_DISPATCH_CMDS_UAV_DESCRIPTOR_SLOT, 
        queue, s_geomBatchDispatchCmdBuffer[queue]);
}
...
static void WriteGeomBatchingDescriptorSet(GEOM_QUEUE queue)
{
    ...
    s_descriptorBuffer.WriteDescriptor(setID, GEOM_BATCH_DRAW_CMD_GEN_DISPATCH_CMD_UAV_DESCRIPTOR_SLOT, 0, s_geomDrawCmdGenDispatchCmdBuffer[queue]);
}
...
void UpdateGPUCommonConstBuffer()
{
    ...
    const glm::float4x4& prevViewProjMatrix = s_mainCamera.GetViewProjMatrixPrev();
    ...
    if (s_cullingTestMode) {
        ...
        constBuff.CULLING_VIEW_PROJ_MATRIX_PREV = prevViewProjMatrix;
    }
...
static void PrecomputeIBLIrradianceMap(vkn::CmdBuffer& cmdBuffer)
{
    ...
    cmdBuffer.CmdDispatch(
        (uint32_t)glm::ceil(COMMON_IRRADIANCE_MAP_SIZE.x / 32.f), 
        (uint32_t)glm::ceil(COMMON_IRRADIANCE_MAP_SIZE.y / 32.f),
        6u
    );
...
static void PrecomputeIBLPrefilteredEnvMap(vkn::CmdBuffer& cmdBuffer)
{
    ...
    for (size_t mip = 0; mip < COMMON_PREFILTERED_ENV_MAP_MIPS_COUNT; ++mip) {
        ...
        cmdBuffer.CmdDispatch(
            (uint32_t)glm::ceil(sizeX / 32.f),
            (uint32_t)glm::ceil(sizeY / 32.f),
            6u
        );
    }
...
static void PrecomputeIBLBRDFIntergrationLUT(vkn::CmdBuffer& cmdBuffer)
{
    ...
    cmdBuffer.CmdDispatch(
        (uint32_t)glm::ceil(COMMON_BRDF_INTEGRATION_LUT_SIZE.x / 32.f), 
        (uint32_t)glm::ceil(COMMON_BRDF_INTEGRATION_LUT_SIZE.y / 32.f),
        1u
    );
...
static void GeomVisIDBufferPass(vkn::CmdBuffer& cmdBuffer)
{
    ...
    for (uint32_t queue = 0; queue < GEOM_QUEUE_COUNT; ++queue) {
        ...
        barriers.AddBufferBarrier(s_geomBatchDispatchCmdBuffer[queue], VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, VK_ACCESS_2_SHADER_WRITE_BIT);
    }
    ...
    cmdBuffer.CmdDispatch((uint32_t)glm::ceil(s_cpuInstData.size() / (float)GEOM_CULLING_CS_GROUP_SIZE), 1u, 1u);
}
...
static void GeomBatchingPass(vkn::CmdBuffer& cmdBuffer, GEOM_QUEUE queue)
{
    ...
    cmdBuffer
        .BeginBarrierList()
            ...
            .AddBufferBarrier(s_geomDrawCmdGenDispatchCmdBuffer[queue], VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, VK_ACCESS_2_SHADER_WRITE_BIT)
            
            .AddBufferBarrier(s_geomBatchDispatchCmdBuffer[queue], VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT, VK_ACCESS_2_INDIRECT_COMMAND_READ_BIT)
        .Push();
    ...
    cmdBuffer.CmdDispatchIndirect(s_geomBatchDispatchCmdBuffer[queue]);
}


static void VisGeomBatchingPass(vkn::CmdBuffer& cmdBuffer)
{
    {
        static constexpr char* markerName = "Vis_Geom_Batching_Pass";
        ...
        {
            ENG_PROFILE_SCOPED_MARKER_C_FMT(eng::ProfileColor::LightSteelBlue1, "Vis_Geom_Batching_Pass_Opaque");
            ENG_PROFILE_GPU_SCOPED_MARKER_C(cmdBuffer, "Vis_Geom_Batching_Pass_Opaque", eng::ProfileColor::LightSteelBlue1);
    
            GeomBatchingPass(cmdBuffer, GEOM_QUEUE_OPAQUE);
        }
    
        {
            ENG_PROFILE_SCOPED_MARKER_C_FMT(eng::ProfileColor::LightSteelBlue1, "Vis_Geom_Batching_Pass_AKill");
            ENG_PROFILE_GPU_SCOPED_MARKER_C(cmdBuffer, "Vis_Geom_Batching_Pass_AKill", eng::ProfileColor::LightSteelBlue1);
...
static void GeomDrawCmdGenPass(vkn::CmdBuffer& cmdBuffer, GEOM_QUEUE queue)
{
    ...
    cmdBuffer
        .BeginBarrierList()
            ...
            .AddBufferBarrier(s_geomDrawCmdGenDispatchCmdBuffer[queue], VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT, VK_ACCESS_2_INDIRECT_COMMAND_READ_BIT)
        .Push();
    ...
    cmdBuffer.CmdDispatchIndirect(s_geomDrawCmdGenDispatchCmdBuffer[queue]);
}


static void VisGeomDrawCmdGenPass(vkn::CmdBuffer& cmdBuffer)
{
    {
        static constexpr char* markerName = "Vis_Geom_Draw_Cmd_Gen_Pass";
        ...
        {
            ENG_PROFILE_SCOPED_MARKER_C_FMT(eng::ProfileColor::ForestGreen, "Vis_Geom_Draw_Cmd_Gen_Pass_Opaque");
            ENG_PROFILE_GPU_SCOPED_MARKER_C(cmdBuffer, "Vis_Geom_Draw_Cmd_Gen_Pass_Opaque", eng::ProfileColor::ForestGreen);
    
            GeomDrawCmdGenPass(cmdBuffer, GEOM_QUEUE_OPAQUE);
        }
    
        {
            ENG_PROFILE_SCOPED_MARKER_C_FMT(eng::ProfileColor::ForestGreen, "Vis_Geom_Draw_Cmd_Gen_Pass_AKill");
            ENG_PROFILE_GPU_SCOPED_MARKER_C(cmdBuffer, "Vis_Geom_Draw_Cmd_Gen_Pass_AKill", eng::ProfileColor::ForestGreen);
...
{
    ENG_PROFILE_SCOPED_MARKER_C_FMT(eng::ProfileColor::DodgerBlue1, "Geom_Culling_Pass");
    ENG_PROFILE_GPU_SCOPED_MARKER_C(cmdBuffer, "Geom_Culling_Pass", eng::ProfileColor::DodgerBlue1);

    GeomCullingClearCounters(cmdBuffer);

    GeomVisIDBufferPass(cmdBuffer);
    VisGeomBatchingPass(cmdBuffer);
    VisGeomDrawCmdGenPass(cmdBuffer);
}
...
void RenderPass_Depth(vkn::CmdBuffer& cmdBuffer, GEOM_QUEUE queue)
{
    ...
    cmdBuffer.CmdBeginRendering(renderInfo);
        ...
        pushConsts.IS_AKILL_PASS = queue == GEOM_QUEUE_AKILL;
...
void VisGeomDepthPass(vkn::CmdBuffer& cmdBuffer)
{
    static constexpr const char* passLabelName = "Vis_Geom_Depth_Pass";
    ...
    {
        static constexpr const char* localPassLabelName = "Vis_Geom_Depth_Pass_Opaque";
    ...
    {
        static constexpr const char* localPassLabelName = "Vis_Geom_Depth_Pass_AKill";
    ...
    if (event.Is<eng::WndKeyEvent>()) {
        ...
        if (keyEvent.key == eng::WndKey::KEY_F5 && keyEvent.IsPressed()) {
            ...
            if (s_cullingTestMode) {
                ...
                s_fixedCamCullViewProjMatrPrev = s_mainCamera.GetViewProjMatrixPrev();
