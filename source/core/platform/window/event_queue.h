...
namespace eng
{
    ...
    enum class WndMouseButtonType : uint8_t
    {
        LEFT,
        RIGHT,
        MIDDLE
    ...
    enum class WndMouseButtonState : uint8_t
    {
        PRESSED,
        RELEASED,
        DOUBLE_CLICKED
    ...
    struct WndMouseButtonEvent : public WndCursorEvent 
    {
        WndMouseButtonEvent(WndMouseButtonType type, WndMouseButtonState state, int16_t x, int16_t y, 
            bool isCtrlDown, bool isShiftDown, bool isLButtonDown, bool isMButtonDown, bool isRButtonDown
        ) : WndCursorEvent(x, y, isCtrlDown, isShiftDown, isLButtonDown, isMButtonDown, isRButtonDown), type(type), state(state) {}
    
        bool IsLeftButton() const { return type == WndMouseButtonType::LEFT; }
        bool IsRightButton() const { return type == WndMouseButtonType::RIGHT; }
        bool IsMiddleButton() const { return type == WndMouseButtonType::MIDDLE; }
    
        bool IsPressed() const { return state == WndMouseButtonState::PRESSED; }
        bool IsReleased() const { return state == WndMouseButtonState::RELEASED; }
        bool IsDoubleClicked() const { return state == WndMouseButtonState::DOUBLE_CLICKED; }
    
        WndMouseButtonType type;
        WndMouseButtonState state;
    };
    ...
    public:
        ...
        requires std::is_constructible_v<WndEventVariant, std::in_place_type_t<EventType>, Args...>
        WndEvent(Args&&... args)
    ...
    const char* WndKeyToStr(WndKey key) noexcept;
}